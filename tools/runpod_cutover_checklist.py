...
def main() -> int:
    ...
    print("6. **L2 MACE / fairchem** — install `runpod-only` extras (`mace-torch`, `fairchem-core`); flip the L2 MLIPManifestAdapter from manifest_only to local_cpu inference once torch is available.")
    print("7. **L4 Solcore** — install on Linux; replace the analytic Shockley-Queisser fallback (electrochem.l4.SolcorePvAdapter) and the tandem analytic (electrochem.l4_tandem_pv) with real Solcore drift-diffusion. Contract preserved.")
    print("8. **L5 OpenMC R2S** — replace the analytic point-kinetics activation (fusion.l5_r2s) with real R2S transport+activation when ALARA / FISPACT-II rights are confirmed.")
    print("9. **L6 BoTorch + Ax** — install `runpod-only` extras; wire the active-learning loop around PyBaMM + PyBOP for autonomous battery design campaigns.")
    print()
